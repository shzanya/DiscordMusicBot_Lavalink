...
class LyricsService:
    ...
    async def get_lyrics(self, title: str, artist: str = "", url: str = "") -> Optional[str]:
        # 🔁 Проверка кеша
        ...
        if self.genius:
            try:
                song = await asyncio.to_thread(self.genius.search_song, title.strip(), artist.strip())
        ...
        if "soundcloud.com" in url:
            try:
                ...
                lyrics = await loop.run_in_executor(None, get_soundcloud_description, url)
                if lyrics:
                    self._lyrics_cache[cache_key] = lyrics
                    return lyrics
            except Exception as e:
