...
class QueueTrack:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requester_name": self.requester.display_name if self.requester else 'Unknown',
...
class QueueManager:
    ...
    async def save_queue(
        ...
        try:
            ...
            update_doc = {
                ...
                "text_channel_id": str(player.text_channel.id) if player.text_channel else "0",
                "message_id": str(player.now_playing_message.id) if player.now_playing_message else "0",
                "volume": getattr(player, 'volume', 100),
                "loop_mode": player.state.loop_mode.value if player.state.loop_mode else "none",
    ...
    async def load_queue(
        ...
        try:
            ...
            if not tracks:
                logger.info(f"No valid tracks found in saved queue for guild {guild_id}")
                return False
            ...
            logger.info(f"Loaded {len(tracks)} tracks from saved queue for guild {guild_id}")
            return True
