...
class SpotifyService:
    ...
    async def _fallback_search(self, url: str) -> wavelink.Playlist | list[wavelink.Playable] | None:
        ...
        try:
            if 'track' in url:
                self.logger.info(f"Fallback: обработка Spotify трека через SoundCloud")
                ...
                self.logger.info(f"Fallback: обработка Spotify плейлиста через SoundCloud")
                ...
                self.logger.info(f"Fallback: обработка Spotify альбома через SoundCloud")
