...
class QueueCommands(commands.Cog, name="📋 Очередь"):
    ...
    async def queue_command(self, ctx, page: int = 1):
        ...
        if not player or not player.queue:
            return await ctx.reply(embed=create_error_embed("Очередь пуста!"))
    ...
    async def clear_command(self, ctx):
        ...
        if not player:
            return await ctx.reply(
                embed=create_error_embed("Бот не подключен к голосовому каналу!")
            )

        player.queue.clear()
        embed = discord.Embed(
            title=f"{Emojis.CLEAR} Очередь очищена", color=Colors.SUCCESS
        )
        await ctx.reply(embed=embed)
    ...
    async def shuffle_command(self, ctx):
        ...
        embed = discord.Embed(
            title=f"{Emojis.SHUFFLE} Очередь перемешана", color=Colors.SUCCESS
        )
        await ctx.reply(embed=embed)
