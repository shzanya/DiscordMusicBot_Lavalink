...
class AudioEffectsManager:
    ...
    EFFECT_CONFIGS = {
        ...
        EffectType.TREMOLO: EffectConfig(
            ...
            filters={"tremolo": {"frequency": 2.0, "depth": 0.5}},
        ),
        EffectType.VIBRATO: EffectConfig(
            name="Вибрато",
            emoji="🎵",
            description="Модуляция частоты",
            filters={"vibrato": {"frequency": 2.0, "depth": 0.5}},
        ),
    ...
    def check_compatibility(
        cls, active_effects: list, new_effect: EffectType
    ) -> tuple[bool, str]:
        ...
        for active_effect in active_effects:
            if active_effect in config.incompatible_with:
                ...
                return (
                    False,
                    f"Несовместим с {active_config.name if active_config else active_effect.value}",
                )
    ...
    async def apply_effects(
        cls, player: HarmonyPlayer, effects: Dict[EffectType, bool]
    ) -> bool:
        try:
            kwargs = {
                "bass": False,
...
class EffectsSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        selected_effects = {EffectType(value): True for value in self.values}
        ...
        for effect in active_effects:
            ...
            compatible, reason = self.effects_manager.check_compatibility(
                other_effects, effect
            )
            if not compatible:
                await interaction.response.send_message(
                    embed=create_error_embed(
                        "Ошибка совместимости", f"Эффект несовместим: {reason}"
                    ),
                    ephemeral=True,
                )
                return
        ...
        if not success:
            await interaction.response.send_message(
                embed=create_error_embed(
                    "Ошибка применения", "Не удалось применить эффекты!"
                ),
                ephemeral=True,
            )
            return
    ...
    def _create_status_embed(self, effects: Dict[EffectType, bool]) -> discord.Embed:
        ...
        if active_effects:
            ...
            embed.add_field(
                name="⚪ Активные эффекты", value="Нет активных эффектов", inline=False
            )
...
class ClearEffectsButton(discord.ui.Button):
    ...
    def __init__(self):
        super().__init__(
            label="Очистить все", style=discord.ButtonStyle.secondary, emoji="🧹"
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not success:
            await interaction.response.send_message(
                embed=create_error_embed(
                    "Ошибка очистки", "Не удалось очистить эффекты!"
                ),
                ephemeral=True,
            )
            return
...
class RefreshButton(discord.ui.Button):
    ...
    def __init__(self):
        super().__init__(
            label="Обновить", style=discord.ButtonStyle.primary, emoji="🔄"
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        current_effects = {}
        for effect_type in EffectType:
            current_effects[effect_type] = getattr(
                view.player.state, effect_type.value, False
            )
...
class EffectsCommands(commands.Cog, name="🎚️ Эффекты"):
    ...
    @discord.app_commands.command(
        name="effects", description="🎚️ Открыть панель управления эффектами"
    )
    async def effects_panel(self, interaction: discord.Interaction):
        ...
        if not player:
            return await interaction.response.send_message(
                embed=create_error_embed(
                    "Ошибка", "Бот не подключен к голосовому каналу!"
                ),
                ephemeral=True,
            )
        ...
        for effect_type in EffectType:
            current_effects[effect_type] = getattr(
                player.state, effect_type.value, False
            )
