...
class EmojiSettings:
    ...
    def __init__(self, color: str = "default", custom_emojis: Optional[Dict[str, str]] = None):
...
class BaseEmojiView(ui.View):
    ...
    def update_emoji(self, custom_id: str, emoji_name: str):
        ...
        for item in self.children:
            if isinstance(item, discord.ui.Button) and getattr(item, 'custom_id', None) == custom_id:
                emoji = self._emoji_settings.get_emoji(emoji_name)
                if emoji and emoji != "❓":
                    item.emoji = emoji
                    item.label = None
